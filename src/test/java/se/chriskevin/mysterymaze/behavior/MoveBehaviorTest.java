...
public class MoveBehaviorTest {
  ...
  public void verifyStopUp() {
    ...
    assertEquals(
        "Y coordinate should av decreased", Point3D.of(0L, -8L, 0L), updatedSprite.position);
    assertEquals("Direction should be UP", Direction.UP, updatedSprite.direction);
    assertEquals(
        "Animation should be WALKING", AnimationState.WALKING, updatedSprite.animationState);
  }
  ...
  public void verifyStopRight() {
    ...
    assertNotEquals("Sprites should not be same", sprite, updatedSprite);
    assertEquals(
        "X coordinate should av increased", Point3D.of(8L, 0L, 0L), updatedSprite.position);
  ...
  public void verifyStopDown() {
    ...
    assertNotEquals("Sprites should not be same", sprite, updatedSprite);
    assertEquals(
        "Y coordinate should av increased", Point3D.of(0L, 8L, 0L), updatedSprite.position);
  ...
  public void verifyStopLeft() {
    ...
    assertNotEquals("Sprites should not be same", sprite, updatedSprite);
    assertEquals(
        "X coordinate should av decreased", Point3D.of(-8L, 0L, 0L), updatedSprite.position);
