...
public class Player {
  ...
  /**
   * This method represents a channel on the soundcard. This is the only channel that can loop a
   * musictrack so it is best used for backgroundmusic. It can be used for a track to play ones. The
   * boolean loop sets if loop is active or not. If true the track loops and if false the track only
   * plays ones. int soundFile represents the number in the initiated array of urls of witch track
   * to be played.
   *
   * @param soundFile
   * @param loop
  ...
  /**
   * This method plays a music file represented by a predfined url in the array soundFileLocation.
   *
   * @param soundFile
   */
