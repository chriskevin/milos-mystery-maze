...
public final class Camera {
  ...
  public static void update(
      final Dimension gameViewSize,
      final Dimension environmentSize,
    ...
    final var targetX =
        adjustCoordinateBoundary(
            half(viewAreaSize.width) - half(viewAreaSize.width),
            target.position.x,
            viewAreaSize.width,
            environmentSize.width);
    final var targetY =
        adjustCoordinateBoundary(
            half(viewAreaSize.height) - half(target.size.height),
            target.position.y,
            viewAreaSize.height,
            environmentSize.height);

    final var offsetP =
        Point3D.of(subtract(targetX, target.position.x), subtract(targetY, target.position.y), 0L);

    final var viewAreaPoint =
        Point3D.of(
    ...
    if (target.colliding) {
      final var collisionZoneLocation =
          Point3D.of(
              addExact(target.position.x, offsetP.x), addExact(target.position.y, offsetP.y), 0L);
      drawCollisionZone(g, AWT.Rectangle.of(collisionZoneLocation, target.size));
    }

    var g2d = (Graphics2D) g;
    g2d.drawImage(
        getImage(imageMapKey(target.animationState, target.direction), target.images),
        targetX.intValue(),
        targetY.intValue(),
        gameView);
  }

  public static Long adjustCoordinateBoundary(
      final Long coordinate,
      final Long targetCoordinate,
      final Long viewAreaMeasurement,
      final Long environmentMeasurement) {
    return (targetCoordinate <= half(viewAreaMeasurement)
            || targetCoordinate >= subtractExact(environmentMeasurement, half(viewAreaMeasurement)))
        ? targetCoordinate
        : coordinate;
  }

  public static List<GameSprite> getVisibleSprites(
      final List<GameSprite> sprites, final Rectangle viewArea) {
