...
import se.chriskevin.mysterymaze.utils.cli.CLI;
import se.chriskevin.mysterymaze.utils.cli.Command;

public final class GameView extends JPanel {

  // Move this
  private Map<Integer, Function<GameSprite, GameSprite>> keyMapStop =
      HashMap.of(
  ...
  private Map<Integer, Function<GameSprite, GameSprite>> keyMapMove =
      HashMap.of(
  ...
  private GameView(final Dimension dimension, final GameEnvironment environment) {
    ...
    final Map<Command, Supplier<String>> commands =
        HashMap.of(Command.NOCLIP, this::noclipOn, Command.GOD, () -> "GOD Mode on");
    ...
    SwingObservable.fromKeyEvents(this)
        ...
        .subscribe(
            (keyEvent) -> {
              final var key = keyEvent.getKeyCode();
              final var pressed = isKeyPressed(keyEvent);
  ...
  private String noclipOn() {
    this.noclip = true;
    return "NOCLIP Mode On";
  }

  private static boolean isKeyPressed(final KeyEvent event) {
    return event.paramString().split(",")[0].equals("KEY_PRESSED");
  }

  static void renderSprite(
      final Graphics g, final GameView gameView, final Point3D offsetP, final GameSprite sprite) {
  ...
  static void drawCollisionZone(final Graphics g, final Rectangle bounds) {
    final var myColour = new Color(255, 0, 0, 128);
    g.setColor(myColour);
    g.fillRect(bounds.x, bounds.y, bounds.width, bounds.height);
  }

  public void enableInput(final boolean enabled) {}
  ...
  private void doDrawing(final Graphics g) {
    final var myColour = new Color(0, 0, 0, 128);
    g.setColor(myColour);
    g.fillRect(0, 0, 1024, 100);

    g.setColor(Color.WHITE);
    g.setFont(new Font("TimesRoman", Font.PLAIN, 20));
    final var ingameMsg = !engine.isPaused() ? "Playing " : "Paused ";
    final var levelMsg = " Level 1";
    final var enemyMsg = " Enemies: " + getByType(SpriteType.ENEMY, environment.sprites).size();
    final var player = getPlayer(environment.sprites);

    final var coordsMsg =
        " X: " + player.position.x + " Y: " + player.position.y + " Colliding: " + player.colliding;

    final var debugMsg = ingameMsg + levelMsg + enemyMsg + coordsMsg;
  ...
  private static GameEnvironment actOnPlayerAction(
      ...
      final Map<Integer, Function<GameSprite, GameSprite>> keyMap,
      final Integer key,
      final GameEnvironment environment,
      final Predicate<GameSprite> checkCollision) {
    if (!engine.isPaused() && keyMap.get(key).isDefined()) {
      final var player = getPlayer(environment.sprites);
      final var updatedPlayer = keyMap.get(key).map(b -> b.apply(player)).getOrNull();

      return checkCollision.test(updatedPlayer)
          ? environment
          : GameEnvironment.of(
  ...
  public static final Function<GameSprite, Predicate<GameSprite>> isColliding =
      target ->
          blocker ->
              AWT.Rectangle.of(blocker.position, blocker.size)
                  .intersects(AWT.Rectangle.of(target.position, target.size));

  public static final Function<List<GameSprite>, Predicate<GameSprite>> anyCollision =
